 ...
 * attendants are busy with the flood of people that suddenly made it
...
public class Day5{
	...
	public static void main(String[] args) throws FileNotFoundException{
		...
		String inputFile = "./Inputs/day_5_input.txt";
		File file = new File(inputFile);

		int mySeat = findSeat(file);
		System.out.println(mySeat);

	}

	public static int findSeat(File file) throws FileNotFoundException{

		String[] boardingPasses = retrieveInput(file);

		int[] seats = new int[lineCounter(file)];
	...
	public static int sanityCheck(File file) throws FileNotFoundException{
		...
		for (String pass: boardingPasses){
			int temp = seatID(pass);
			if (temp > highestSeatID){
				highestSeatID = temp;
			}
		}
	...
	public static int seatID(String pass){
		...
		int lowestRow = 0;
		int highestRow = 127;
		int midRow = (lowestRow + highestRow) / 2;

		int seatColumn;
		int lowestColumn = 0;
		int highestColumn = 7;
		int midColumn = (lowestColumn + highestColumn) / 2;
		...
		for (char letter: pass.toCharArray()){
			...
			if (i < 7){
				...
				midRow = (lowestRow + highestRow) / 2;
			}

			if (i >= 7){

				if (letter == 'L'){
					highestColumn = midColumn;
				}

				else if (letter == 'R'){
					lowestColumn = midColumn + 1;
				}

				midColumn = (lowestColumn + highestColumn) / 2;
			}
	...
	public static String[] retrieveInput(File file) throws FileNotFoundException{

		Scanner scRetrieve = new Scanner(file);

		String[] boardingPasses = new String[lineCounter(file)];

		int i = 0;
		for (String pass: boardingPasses){
			boardingPasses[i] = scRetrieve.nextLine();
			i++;
		}
