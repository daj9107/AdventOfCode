...
public class Day7 {
	...
	public static void main(String[] args) throws FileNotFoundException{
		...
		String sampleFile = "day_7_sample.txt";
		String inputFile = "day_7_input.txt";
		File sample = new File(sampleFile);
		File file = new File(inputFile);

		LinkedList<Rules> ruleset = retrieveRules(file);

		String targetColor = "shinygold";
		
		int partOneCounter = 0;
		for (Rules rule : ruleset){
			if (rule.findContents(targetColor)){
				partOneCounter++;
			}
		...
		ruleset = retrieveRules(file);

		int partTwo = 0;
		for (Rules rule : ruleset){
			if (rule.color.equals(targetColor)){
				partTwo = rule.mustContain();
			}
		}
	...
	public static LinkedList<Rules> retrieveRules(File file) throws FileNotFoundException{

		LinkedList<Rules> ruleset = new LinkedList<Rules>();
		...
		for (String line : lines){
			ruleset.add(new Rules(line, lines));
		}
